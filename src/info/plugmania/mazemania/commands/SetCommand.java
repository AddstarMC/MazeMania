...
public class SetCommand {
	...
	public boolean handle(CommandSender sender, String[] args) {
		...
		if (args.length <= 1) {
			...
			sender.sendMessage(Util.formatMessage("/maze set lobby        - Set the arena lobby (waiting area)"));
	...
	public boolean triggerHandle(CommandSender sender, String[] args) {
		...
		if (args.length <= 1) {
			...
			sender.sendMessage(Util.formatMessage("Lightning, Fire, Poison, Blind, Speed, Night, Invisible,"));
			sender.sendMessage(Util.formatMessage("Zombie, Spider, Cavespider"));
			sender.sendMessage(Util.formatMessage("Setup Commands:"));
			...
			if (args[1].equalsIgnoreCase("add")) {
				if (args.length > 2) {
					...
					if (args[2].equalsIgnoreCase("lightning")) {
						...
						events.add("poison");
					} else if (args[2].equalsIgnoreCase("blind")) {
						events.add("blind");
					} else if (args[2].equalsIgnoreCase("confuse")) {
						events.add("confuse");
					} else if (args[2].equalsIgnoreCase("slow")) {
						events.add("slow");
						...
						events.add("invisible");
					} else if (args[2].equalsIgnoreCase("zombie")) {
						events.add("zombie");
					} else if (args[2].equalsIgnoreCase("spider")) {
						events.add("spide");
					} else if (args[2].equalsIgnoreCase("cavespider")) {
						events.add("cavespider");
					} else {
						sender.sendMessage(Util.formatMessage("Invalid trigger type."));
						return false;
					}
