...
public Effects(MazeMania instance){
	...
	public void apply(Player p,String arguments,String effect){
		try {
			...
			Util.debug("Error in " + effect + " trigger reflection: " + e.getMessage() + " (" + p.getName() + ")");
			//e.printStackTrace();
		}
	}
	...
	@effect(argument="4",argUsage="<seconds>") public void blind(Player p,String args) {
		p.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, Integer.parseInt(args) * 20, 1));
	}

	@effect(argument="4",argUsage="<seconds>") public void confuse(Player p,String args) {
		p.addPotionEffect(new PotionEffect(PotionEffectType.CONFUSION, Integer.parseInt(args) * 20, 1));
	}

	@effect(argument="4",argUsage="<seconds>") public void slow(Player p,String args) {
		p.addPotionEffect(new PotionEffect(PotionEffectType.SLOW, Integer.parseInt(args) * 20, 2));
	}

	@effect(argument="4",argUsage="<seconds>") public void speed(Player p,String args) {
		p.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, Integer.parseInt(args) * 20, 2));
	}

	@effect(argument="4",argUsage="<seconds>") public void night(Player p,String args) {
		p.addPotionEffect(new PotionEffect(PotionEffectType.NIGHT_VISION, Integer.parseInt(args) * 20, 5));
	}

	@effect(argument="4",argUsage="<seconds>") public void invisible(Player p,String args) {
		p.addPotionEffect(new PotionEffect(PotionEffectType.INVISIBILITY, Integer.parseInt(args) * 20, 1));
	}
