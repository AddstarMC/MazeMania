...
public class Reward {
	...
	public void rewardPlayer(Player p) {
		...
		if (plugin.mainConf.getBoolean("economy_rewards", true)) {
			...
			p.sendMessage(Util.formatMessage("You have been rewarded with $" + plugin.mainConf.getDouble("economy_reward_amount", 20.0D)));
		}
		if (plugin.mainConf.getBoolean("item_rewards", true)) {
			rewardPlayerItem(p);
			p.sendMessage(Util.formatMessage("You have been rewarded with items!"));
		}
	}
